...
* 돼지가 벽으로 몰릴 경우, 혼자 버벅거리는 현상이 있다. 길을 찾지 못하는 듯 하다.
* 달릴 때, 스태미너 소모가 지속적으로 일어나지 않는다.
GetKeyDown, GetKey
* Stair를 설치할 때, 설치 할 수 없는 곳에 프리팹의 색이 빨강으로 변하지 않는다.

-------------------------------------------------------------------

## 개발하고 싶은 것
* 슈퍼 몬스터를 잡았을 때, 평소보다 더 많은 아이템이 펑 하고 터져나오도록 만들고 싶다. -> 일렬로 나오는 현상을 수정
-> 고기 아이템에 RigidBody가 있으니 AddForce를 이용해보자
