...
{
    [SerializeField]
    private int rockMaxCount; // 바위 최대 생성 갯수
    ...
    [SerializeField]
    private float destroyTime; //바위 파괴후 파편 파괴 대기시간

    [SerializeField]
    private SphereCollider sphereCollider;//남아있는 컬라이더를 비활성화 시킬 목적


    [SerializeField]
    private string strike_Sound; //필요한 사운드 이름
    [SerializeField]
    private string destroy_Sound; //필요한 파괴사운드 이름
    ...
    [SerializeField]
    private GameObject go_RockItem_Prefab; //돌멩이 아이템
